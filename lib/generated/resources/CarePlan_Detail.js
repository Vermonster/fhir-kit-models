...
class CarePlan_Detail extends BackboneElement {
  ...
  get scheduledTiming() {
    if (!this[_values].scheduledTiming) {
  ...
  get scheduledPeriod() {
    if (!this[_values].scheduledPeriod) {
  ...
  get _scheduledString() {
    if (!this[_values]._scheduledString) {
      const klass = require('./Element');
