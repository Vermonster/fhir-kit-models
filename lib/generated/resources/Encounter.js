...
class Encounter extends DomainResource {
  ...
  get statusHistory() {
    if (!this[_values].statusHistory) {
      const klass = require('./Encounter_StatusHistory');
  ...
  get classHistory() {
    if (!this[_values].classHistory) {
      const klass = require('./Encounter_ClassHistory');
  ...
  get participant() {
    if (!this[_values].participant) {
      const klass = require('./Encounter_Participant');
  ...
  get diagnosis() {
    if (!this[_values].diagnosis) {
      const klass = require('./Encounter_Diagnosis');
  ...
  get hospitalization() {
    if (!this[_values].hospitalization) {
      const klass = require('./Encounter_Hospitalization');
  ...
  get location() {
    if (!this[_values].location) {
      const klass = require('./Encounter_Location');
  ...
  get serviceProvider() {
    if (!this[_values].serviceProvider) {
      const klass = require('./Reference');
