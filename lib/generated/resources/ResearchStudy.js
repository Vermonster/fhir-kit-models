...
class ResearchStudy extends DomainResource {
  ...
  get principalInvestigator() {
    if (!this[_values].principalInvestigator) {
      const klass = require('./Reference');
  ...
  get arm() {
    if (!this[_values].arm) {
      const klass = require('./ResearchStudy_Arm');
