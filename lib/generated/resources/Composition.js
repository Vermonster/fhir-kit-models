...
class Composition extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof Composition || values instanceof DomainResource) {
  ...
  get _confidentiality() {
    if (!this[_values]._confidentiality) {
      const klass = require('./Element');
  ...
  get attester() {
    if (!this[_values].attester) {
      const klass = require('./Composition_Attester');
  ...
  get relatesTo() {
    if (!this[_values].relatesTo) {
      const klass = require('./Composition_RelatesTo');
  ...
  get event() {
    if (!this[_values].event) {
      const klass = require('./Composition_Event');
