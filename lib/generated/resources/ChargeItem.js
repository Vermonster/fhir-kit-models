...
class ChargeItem extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof ChargeItem || values instanceof DomainResource) {
  ...
  get participant() {
    if (!this[_values].participant) {
      const klass = require('./ChargeItem_Participant');
  ...
  get performingOrganization() {
    if (!this[_values].performingOrganization) {
      const klass = require('./Reference');
  ...
  get requestingOrganization() {
    if (!this[_values].requestingOrganization) {
      const klass = require('./Reference');
  ...
  get _factorOverride() {
    if (!this[_values]._factorOverride) {
      const klass = require('./Element');
  ...
  get _overrideReason() {
    if (!this[_values]._overrideReason) {
      const klass = require('./Element');
