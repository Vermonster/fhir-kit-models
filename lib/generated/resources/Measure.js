...
class Measure extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof Measure || values instanceof DomainResource) {
  ...
  get _lastReviewDate() {
    if (!this[_values]._lastReviewDate) {
      const klass = require('./Element');
  ...
  get effectivePeriod() {
    if (!this[_values].effectivePeriod) {
  ...
  get relatedArtifact() {
    if (!this[_values].relatedArtifact) {
  ...
  get compositeScoring() {
    if (!this[_values].compositeScoring) {
      const klass = require('./CodeableConcept');
  ...
  get _riskAdjustment() {
    if (!this[_values]._riskAdjustment) {
      const klass = require('./Element');
  ...
  get _rateAggregation() {
    if (!this[_values]._rateAggregation) {
      const klass = require('./Element');
  ...
  get _clinicalRecommendationStatement() {
    if (!this[_values]._clinicalRecommendationStatement) {
      const klass = require('./Element');
  ...
  get _improvementNotation() {
    if (!this[_values]._improvementNotation) {
      const klass = require('./Element');
  ...
  get group() {
    if (!this[_values].group) {
      const klass = require('./Measure_Group');
  ...
  get supplementalData() {
    if (!this[_values].supplementalData) {
      const klass = require('./Measure_SupplementalData');
