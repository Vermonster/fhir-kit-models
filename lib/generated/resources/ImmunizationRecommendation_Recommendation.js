...
class ImmunizationRecommendation_Recommendation extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof ImmunizationRecommendation_Recommendation || values instanceof BackboneElement) {
  ...
  get forecastStatus() {
    if (!this[_values].forecastStatus) {
      const klass = require('./CodeableConcept');
  ...
  get dateCriterion() {
    if (!this[_values].dateCriterion) {
      const klass = require('./ImmunizationRecommendation_DateCriterion');
  ...
  get protocol() {
    if (!this[_values].protocol) {
      const klass = require('./ImmunizationRecommendation_Protocol');
  ...
  get supportingImmunization() {
    if (!this[_values].supportingImmunization) {
  ...
  get supportingPatientInformation() {
    if (!this[_values].supportingPatientInformation) {
      const klass = require('./Reference');
