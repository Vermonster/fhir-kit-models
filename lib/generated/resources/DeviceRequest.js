...
class DeviceRequest extends DomainResource {
  ...
  get codeCodeableConcept() {
    if (!this[_values].codeCodeableConcept) {
      const klass = require('./CodeableConcept');
  ...
  get requester() {
    if (!this[_values].requester) {
      const klass = require('./DeviceRequest_Requester');
