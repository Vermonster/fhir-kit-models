...
class ExplanationOfBenefit extends DomainResource {
  ...
  get billablePeriod() {
    if (!this[_values].billablePeriod) {
  ...
  get originalPrescription() {
    if (!this[_values].originalPrescription) {
      const klass = require('./Reference');
  ...
  get employmentImpacted() {
    if (!this[_values].employmentImpacted) {
      const klass = require('./Period');
  ...
  get hospitalization() {
    if (!this[_values].hospitalization) {
      const klass = require('./Period');
  ...
  get unallocDeductable() {
    if (!this[_values].unallocDeductable) {
      const klass = require('./Money');
  ...
  get benefitBalance() {
    if (!this[_values].benefitBalance) {
