...
class CompartmentDefinition extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof CompartmentDefinition || values instanceof DomainResource) {
  ...
  set search(value) {
    return this[_values].search = value;
  }
  get _search() {
    if (!this[_values]._search) {
      const klass = require('./Element');
      this[_values]._search = new klass();
    }
    return this[_values]._search;
  }
  set _search(value) {
    const klass = require('./Element');
    return this[_values]._search = new klass(value);
  }
  get resource() {
    if (!this[_values].resource) {
      const klass = require('./CompartmentDefinition_Resource');
