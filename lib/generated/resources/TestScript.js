...
class TestScript extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof TestScript || values instanceof DomainResource) {
  ...
  get origin() {
    if (!this[_values].origin) {
      const klass = require('./TestScript_Origin');
  ...
  get destination() {
    if (!this[_values].destination) {
      const klass = require('./TestScript_Destination');
  ...
  get metadata() {
    if (!this[_values].metadata) {
      const klass = require('./TestScript_Metadata');
  ...
  get fixture() {
    if (!this[_values].fixture) {
      const klass = require('./TestScript_Fixture');
  ...
  get variable() {
    if (!this[_values].variable) {
      const klass = require('./TestScript_Variable');
  ...
  get rule() {
    if (!this[_values].rule) {
      const klass = require('./TestScript_Rule');
  ...
  get ruleset() {
    if (!this[_values].ruleset) {
      const klass = require('./TestScript_Ruleset');
  ...
  get setup() {
    if (!this[_values].setup) {
      const klass = require('./TestScript_Setup');
  ...
  get test() {
    if (!this[_values].test) {
      const klass = require('./TestScript_Test');
  ...
  get teardown() {
    if (!this[_values].teardown) {
      const klass = require('./TestScript_Teardown');
