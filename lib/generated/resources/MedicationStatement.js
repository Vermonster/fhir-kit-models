...
class MedicationStatement extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof MedicationStatement || values instanceof DomainResource) {
  ...
  get informationSource() {
    if (!this[_values].informationSource) {
      const klass = require('./Reference');
  ...
  get reasonNotTaken() {
    if (!this[_values].reasonNotTaken) {
      const klass = require('./CodeableConcept');
