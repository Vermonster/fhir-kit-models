...
class CarePlan extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof CarePlan || values instanceof DomainResource) {
  ...
  get supportingInfo() {
    if (!this[_values].supportingInfo) {
      const klass = require('./Reference');
  ...
  get activity() {
    if (!this[_values].activity) {
      const klass = require('./CarePlan_Activity');
