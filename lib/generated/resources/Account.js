...
class Account extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof Account || values instanceof DomainResource) {
  ...
  get coverage() {
    if (!this[_values].coverage) {
      const klass = require('./Account_Coverage');
  ...
  get guarantor() {
    if (!this[_values].guarantor) {
      const klass = require('./Account_Guarantor');
