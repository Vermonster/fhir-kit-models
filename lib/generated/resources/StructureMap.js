...
class StructureMap extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof StructureMap || values instanceof DomainResource) {
  ...
  get structure() {
    if (!this[_values].structure) {
      const klass = require('./StructureMap_Structure');
      this[_values].structure = ArrayProxy(klass);
    }
    return this[_values].structure;
  }
  set structure(value) {
    const klass = require('./StructureMap_Structure');
    this[_values].structure = ArrayProxy(klass);
    value.forEach(entry => this[_values].structure.push(entry));
    return this[_values].structure;
  }
  get import() {
    return this[_values].import;
  }
  set import(value) {
    return this[_values].import = value;
  }
  get _import() {
    if (!this[_values]._import) {
      const klass = require('./Element');
      this[_values]._import = ArrayProxy(klass);
    }
    return this[_values]._import;
  }
  set _import(value) {
    const klass = require('./Element');
    this[_values]._import = ArrayProxy(klass);
    value.forEach(entry => this[_values]._import.push(entry));
    return this[_values]._import;
  }
  get group() {
    if (!this[_values].group) {
      const klass = require('./StructureMap_Group');
