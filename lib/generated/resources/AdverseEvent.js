...
class AdverseEvent extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof AdverseEvent || values instanceof DomainResource) {
  ...
  get eventParticipant() {
    if (!this[_values].eventParticipant) {
      const klass = require('./Reference');
  ...
  get suspectEntity() {
    if (!this[_values].suspectEntity) {
      const klass = require('./AdverseEvent_SuspectEntity');
  ...
  get subjectMedicalHistory() {
    if (!this[_values].subjectMedicalHistory) {
      const klass = require('./Reference');
  ...
  get referenceDocument() {
    if (!this[_values].referenceDocument) {
      const klass = require('./Reference');
