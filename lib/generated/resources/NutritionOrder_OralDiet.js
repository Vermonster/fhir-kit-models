...
class NutritionOrder_OralDiet extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof NutritionOrder_OralDiet || values instanceof BackboneElement) {
  ...
  get nutrient() {
    if (!this[_values].nutrient) {
      const klass = require('./NutritionOrder_Nutrient');
  ...
  get texture() {
    if (!this[_values].texture) {
      const klass = require('./NutritionOrder_Texture');
  ...
  get fluidConsistencyType() {
    if (!this[_values].fluidConsistencyType) {
      const klass = require('./CodeableConcept');
