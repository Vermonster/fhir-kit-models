...
class ImagingStudy extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof ImagingStudy || values instanceof DomainResource) {
  ...
  get _numberOfSeries() {
    if (!this[_values]._numberOfSeries) {
      const klass = require('./Element');
  ...
  get procedureReference() {
    if (!this[_values].procedureReference) {
      const klass = require('./Reference');
  ...
  get series() {
    if (!this[_values].series) {
      const klass = require('./ImagingStudy_Series');
