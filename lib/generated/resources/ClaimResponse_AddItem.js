...
class ClaimResponse_AddItem extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof ClaimResponse_AddItem || values instanceof BackboneElement) {
  ...
  get adjudication() {
    if (!this[_values].adjudication) {
      const klass = require('./ClaimResponse_Adjudication');
  ...
  get detail() {
    if (!this[_values].detail) {
      const klass = require('./ClaimResponse_Detail1');
