...
class CodeSystem extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof CodeSystem || values instanceof DomainResource) {
  ...
  get _caseSensitive() {
    if (!this[_values]._caseSensitive) {
      const klass = require('./Element');
  ...
  get _hierarchyMeaning() {
    if (!this[_values]._hierarchyMeaning) {
      const klass = require('./Element');
  ...
  get _compositional() {
    if (!this[_values]._compositional) {
      const klass = require('./Element');
  ...
  get _versionNeeded() {
    if (!this[_values]._versionNeeded) {
      const klass = require('./Element');
  ...
  get filter() {
    if (!this[_values].filter) {
      const klass = require('./CodeSystem_Filter');
  ...
  get property() {
    if (!this[_values].property) {
      const klass = require('./CodeSystem_Property');
