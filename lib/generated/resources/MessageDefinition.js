...
class MessageDefinition extends DomainResource {
  ...
  get focus() {
    if (!this[_values].focus) {
      const klass = require('./MessageDefinition_Focus');
  ...
  get _responseRequired() {
    if (!this[_values]._responseRequired) {
      const klass = require('./Element');
  ...
  get allowedResponse() {
    if (!this[_values].allowedResponse) {
      const klass = require('./MessageDefinition_AllowedResponse');
