...
const generateClass = (schema, outputPath, template) => {
  for (let [className, definition] of Object.entries(schema.definitions)) {
    className = className.replace('_', '');
    console.log(`Generating: ${className}.js`);
    let superClass;
    if (definition.allOf[0].$ref) {
      superClass = definition.allOf[0].$ref.slice(definition.allOf[0].$ref.lastIndexOf('/') + 1).replace('_', '');
    }
    ...
    renderClass({ className, superClass, usesArrayProxy, params }, outputPath, template);
  }
};
