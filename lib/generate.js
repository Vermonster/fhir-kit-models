...
const loadTemplates = () => {
  ...
  for (const templateFileName of templateFileNames) {
    const src = fs.readFileSync(path.normalize(`${__dirname}/../templates/${templateFileName}`), 'utf8');
...
const renderClass = (params, outputPath) => {
  dust.render('element-template', params, (err, out) => {
    if (err) {
      console.log(err);
      return;
    }
    ...
    fs.writeFileSync(path.normalize(`${outputPath}/${params.className}.js`), prettyJS);
  });
};
...
const createClasses = () => {
  ...
  generateClasses(schemaFileNames, `${__dirname}/generated/resources`);
};
