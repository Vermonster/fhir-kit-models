...
class GraphDefinitionTarget extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof GraphDefinitionTarget || values instanceof BackboneElement) {
  ...
  get compartment() {
    if (!isPresent(this[_values].compartment)) {
      const klass = require('./GraphDefinitionCompartment');
  ...
  get link() {
    if (!isPresent(this[_values].link)) {
      const klass = require('./GraphDefinitionLink');
