...
class QuestionnaireItem extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof QuestionnaireItem || values instanceof BackboneElement) {
  ...
  get enableWhen() {
    if (!isPresent(this[_values].enableWhen)) {
      const klass = require('./QuestionnaireEnableWhen');
  ...
  get option() {
    if (!isPresent(this[_values].option)) {
      const klass = require('./QuestionnaireOption');
  ...
  get item() {
    if (!isPresent(this[_values].item)) {
      const klass = require('./QuestionnaireItem');
