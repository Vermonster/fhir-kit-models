...
class OperationDefinition extends DomainResource {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof OperationDefinition ||
  ...
  get _idempotent() {
    if (!isPresent(this[_values]._idempotent)) {
      const klass = require('./Element');
  ...
  get _instance() {
    if (!isPresent(this[_values]._instance)) {
      const klass = require('./Element');
  ...
  get overload() {
    if (!isPresent(this[_values].overload)) {
      const klass = require('./OperationDefinitionOverload');
