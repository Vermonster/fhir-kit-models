...
class SequenceReferenceSeq extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof SequenceReferenceSeq || values instanceof BackboneElement) {
  ...
  get chromosome() {
    if (!isPresent(this[_values].chromosome)) {
  ...
  get _genomeBuild() {
    if (!isPresent(this[_values]._genomeBuild)) {
  ...
  get referenceSeqId() {
    if (!isPresent(this[_values].referenceSeqId)) {
      const klass = require('./CodeableConcept');
  ...
  get referenceSeqPointer() {
    if (!isPresent(this[_values].referenceSeqPointer)) {
  ...
  get _referenceSeqString() {
    if (!isPresent(this[_values]._referenceSeqString)) {
      const klass = require('./Element');
  ...
  get _strand() {
    if (!isPresent(this[_values]._strand)) {
      const klass = require('./Element');
  ...
  get _windowStart() {
    if (!isPresent(this[_values]._windowStart)) {
      const klass = require('./Element');
  ...
  get _windowEnd() {
    if (!isPresent(this[_values]._windowEnd)) {
      const klass = require('./Element');
