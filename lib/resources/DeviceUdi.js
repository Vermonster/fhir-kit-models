...
class DeviceUdi extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof DeviceUdi || values instanceof BackboneElement) {
  ...
  get _deviceIdentifier() {
    if (!isPresent(this[_values]._deviceIdentifier)) {
  ...
  get _jurisdiction() {
    if (!isPresent(this[_values]._jurisdiction)) {
      const klass = require('./Element');
  ...
  get _carrierHRF() {
    if (!isPresent(this[_values]._carrierHRF)) {
      const klass = require('./Element');
  ...
  get _carrierAIDC() {
    if (!isPresent(this[_values]._carrierAIDC)) {
      const klass = require('./Element');
  ...
  get _issuer() {
    if (!isPresent(this[_values]._issuer)) {
      const klass = require('./Element');
  ...
  get _entryType() {
    if (!isPresent(this[_values]._entryType)) {
      const klass = require('./Element');
