...
class ImagingStudySeries extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof ImagingStudySeries || values instanceof BackboneElement) {
  ...
  get modality() {
    if (!isPresent(this[_values].modality)) {
  ...
  get laterality() {
    if (!isPresent(this[_values].laterality)) {
      const klass = require('./Coding');
  ...
  get instance() {
    if (!isPresent(this[_values].instance)) {
      const klass = require('./ImagingStudyInstance');
