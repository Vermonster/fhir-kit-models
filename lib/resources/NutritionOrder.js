...
class NutritionOrder extends DomainResource {
  constructor(values = {}) {
    ...
    if (values instanceof NutritionOrder ||
  ...
  get oralDiet() {
    if (!isPresent(this[_values].oralDiet)) {
      const klass = require('./NutritionOrderOralDiet');
  ...
  get supplement() {
    if (!isPresent(this[_values].supplement)) {
      const klass = require('./NutritionOrderSupplement');
  ...
  get enteralFormula() {
    if (!isPresent(this[_values].enteralFormula)) {
      const klass = require('./NutritionOrderEnteralFormula');
