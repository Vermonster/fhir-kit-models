...
class SpecimenContainer extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof SpecimenContainer || values instanceof BackboneElement) {
  ...
  get capacity() {
    if (!isPresent(this[_values].capacity)) {
  ...
  get specimenQuantity() {
    if (!isPresent(this[_values].specimenQuantity)) {
      const klass = require('./Quantity');
  ...
  get additiveCodeableConcept() {
    if (!isPresent(this[_values].additiveCodeableConcept)) {
      const klass = require('./CodeableConcept');
  ...
  get additiveReference() {
    if (!isPresent(this[_values].additiveReference)) {
