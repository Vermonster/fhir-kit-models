...
class CodeSystemConcept extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof CodeSystemConcept || values instanceof BackboneElement) {
  ...
  get designation() {
    if (!isPresent(this[_values].designation)) {
      const klass = require('./CodeSystemDesignation');
  ...
  get property() {
    if (!isPresent(this[_values].property)) {
      const klass = require('./CodeSystemProperty1');
