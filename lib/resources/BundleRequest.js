...
class BundleRequest extends BackboneElement {
  constructor(values = {}) {
    super();
    this[_values] = {};
    if (values instanceof BundleRequest || values instanceof BackboneElement) {
  ...
  get _method() {
    if (!isPresent(this[_values]._method)) {
  ...
  get _ifNoneMatch() {
    if (!isPresent(this[_values]._ifNoneMatch)) {
      const klass = require('./Element');
  ...
  get _ifModifiedSince() {
    if (!isPresent(this[_values]._ifModifiedSince)) {
      const klass = require('./Element');
  ...
  get _ifMatch() {
    if (!isPresent(this[_values]._ifMatch)) {
      const klass = require('./Element');
  ...
  get _ifNoneExist() {
    if (!isPresent(this[_values]._ifNoneExist)) {
      const klass = require('./Element');
