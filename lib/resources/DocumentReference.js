...
class DocumentReference extends DomainResource {
  constructor(values = {}) {
    ...
    if (values instanceof DocumentReference ||
  ...
  get relatesTo() {
    if (!isPresent(this[_values].relatesTo)) {
      const klass = require('./DocumentReferenceRelatesTo');
  ...
  get content() {
    if (!isPresent(this[_values].content)) {
      const klass = require('./DocumentReferenceContent');
  ...
  get context() {
    if (!isPresent(this[_values].context)) {
      const klass = require('./DocumentReferenceContext');
